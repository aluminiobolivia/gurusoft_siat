...
class AccountInvoice(models.Model):
    ...
    pago_id = fields.Many2one('siat.metodo.pago', string=u'Método Pago', copy=False)
    anulacion_id = fields.Many2one('siat.motivo.anulacion', string=u'Motivo de anulación', copy=False)
    doc_identidad_id = fields.Many2one('siat.documento.identidad', string=u'Tipo Doc. Identidad', copy=False)
    emision_id = fields.Many2one('siat.tipo.emision', string=u'Tipo Emisión', copy=False)
            ...
            if inv.token_id:
                ...
                for lines in inv.invoice_line_ids:
                    ...
                    if not lines.product_id.siat_unidad_id:
                        raise UserError(_("Defina Unidad de Medida SIAT en el producto: %s") % (lines.product_id.name))
                    ...
                    val_line = {
                        ...
                        "UnidadMedida": u''.join(lines.product_id.siat_unidad_id.codigo_clasificador).encode('utf-8'),
                        "PrecioUnitario": price_unit_sin,
                        "SubTotal": total,
                        "Descripcion": u''.join(lines.name).encode('utf-8'),
                        # "MontoDescuento": round(total_factura_sin - total_factura, 2),
                        "MontoDescuento": round(total_desc_a, 2),
                    }
                    detalles.append(val_line)
                total_factura = round(total_factura, 2)
                total_factura_desc = round(total_factura_sin - total_factura, 2)
                ...
                invoice = {
                    ...
                    # "DescuentoAdicional": total_factura_desc,
                    "NombreIntegracion": "INTEGRACION EDOC",
                }
